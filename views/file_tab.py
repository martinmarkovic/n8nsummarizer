...
class FileTab(BaseTab):
    ...
    def _setup_file_selection_section(self):
        ...
        browse_btn = ttk.Button(
            button_frame, text="Browse File", command=self._browse_file
        )
    ...
    def _setup_webhook_section(self):
        ...
        self.webhook_entry = ttk.Entry(
            webhook_frame, textvariable=self.custom_webhook_var
        )
    ...
    def _setup_content_response_section(self):
        ...
        content_frame = ttk.LabelFrame(
            content_response_frame, text="Content Preview & Edit", padding="10"
        )
        content_frame.grid(
        ...
        response_frame = ttk.LabelFrame(
            content_response_frame, text="n8n Response", padding="10"
        )
        response_frame.grid(
    ...
    def _setup_action_bar(self):
        ...
        self.send_btn = ttk.Button(
            bottom_frame, text="Send to n8n", command=self._send_clicked
        )
        ...
        ttk.Checkbutton(
            export_controls_frame, text="Auto .txt", variable=self.auto_export_txt_var
        ).grid(row=0, column=1, sticky=tk.W, padx=(0, 10))

        ttk.Checkbutton(
            export_controls_frame, text="Auto .docx", variable=self.auto_export_docx_var
        ).grid(row=0, column=2, sticky=tk.W, padx=(0, 20))

        ttk.Label(export_controls_frame, text="Export:").grid(
            row=0, column=3, sticky=tk.W, padx=(0, 5)
        )

        self.export_txt_btn = ttk.Button(
            export_controls_frame, text="📄 .txt", command=self._export_txt_clicked
        )
        self.export_txt_btn.grid(row=0, column=4, padx=(0, 5))

        self.export_docx_btn = ttk.Button(
            export_controls_frame, text="📝 .docx", command=self._export_docx_clicked
        )
        self.export_docx_btn.grid(row=0, column=5)

        # Summarize Textbox button
        self.summarize_textbox_btn = ttk.Button(
            export_controls_frame,
            text="📋 Summarize Textbox",
            command=lambda: self._send_clicked(True),
            state=tk.NORMAL,
        )
        self.summarize_textbox_btn.grid(row=0, column=6, padx=(10, 0))

        # Clear button
        self.clear_btn = ttk.Button(
            bottom_frame, text="Clear All", command=self._clear_clicked
        )
    ...
    def set_file_info(self, info_dict: dict):
        ...
        if info_dict:
            ...
            info_text += (
                f"Characters (no spaces): {info_dict.get('characters_no_spaces', 0)}\n"
            )
