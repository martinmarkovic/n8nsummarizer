...
class FileController:
    ...
    def handle_send_clicked(self, allow_textbox_only=False):
        ...
        if not gui_webhook_url or not gui_webhook_url.strip():
            self.view.show_error(
                "Webhook URL in GUI is empty! Please enter a valid webhook URL."
            )
            return
        ...
        thread = threading.Thread(
            ...
            args=(
                ...
                content,
                gui_webhook_url,
                webhook_override["override"],
            ),
            daemon=True,
        )
        thread.start()

    def _send_to_n8n_thread(
        self,
        file_path: str,
        file_info: dict,
        content: str,
        ...
        try:
            ...
            logger.debug(
                f"File size: {file_size_bytes} bytes ({file_size_bytes / 1024:.1f} KB)"
            )
            ...
            success, summary, error = self.n8n_model.send_content(
                ...
                metadata={"size_bytes": file_info["size"], "lines": file_info["lines"]},
            )
            ...
            if success:
                ...
                if summary:
                    ...
                    self.view.root.after(
                        0, self._on_success_no_summary, file_info["name"], saved_msg
                    )
            else:
    ...
    def handle_export_txt(self, manual_call: bool = True):
        ...
        if export_prefs["use_original_location"] and export_prefs["original_directory"]:
            ...
            file_path = os.path.join(
                export_prefs["original_directory"], default_filename
            )
        ...
        if success:
            if not silent:
                self.view.show_success(
                    f"Response exported successfully to:\n{file_path}"
                )
    ...
    def handle_export_docx(self):
        ...
        if export_prefs["original_basename"]:
            ...
            default_filename = self.file_model.generate_timestamp_filename(
                format="docx"
            )
    ...
    def _auto_export_response(
        self, response_content: str, export_txt: bool = True, export_docx: bool = True
    ):
    ...
    def _save_export_preferences_to_env(self, export_prefs: dict):
        ...
        found = {"use_original_location": False, "auto_txt": False, "auto_docx": False}

        new_lines = []
        for line in env_lines:
            if line.strip().startswith("EXPORT_USE_ORIGINAL_LOCATION="):
                new_lines.append(
                    f"EXPORT_USE_ORIGINAL_LOCATION={'true' if export_prefs['use_original_location'] else 'false'}\n"
                )
                found["use_original_location"] = True
            elif line.strip().startswith("EXPORT_AUTO_TXT="):
                new_lines.append(
                    f"EXPORT_AUTO_TXT={'true' if export_prefs['auto_export_txt'] else 'false'}\n"
                )
                found["auto_txt"] = True
            elif line.strip().startswith("EXPORT_AUTO_DOCX="):
                new_lines.append(
                    f"EXPORT_AUTO_DOCX={'true' if export_prefs['auto_export_docx'] else 'false'}\n"
                )
    ...
    def test_n8n_connection(self) -> bool:
        ...
        if is_connected:
            ...
            self.view.show_error(
                "Cannot reach n8n server. Check if it's running at the configured URL."
            )
